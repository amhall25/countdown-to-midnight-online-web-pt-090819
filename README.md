...
4. Our Ruby program executes so quickly that it doesn't actually count down at the speed of one second per number. See if you can make the loop pausckoverflow.com/questions/1329967/tell-ruby-program-to-waite for one second each trip around ([hint](http://sta-some-amount-of-time)). Write this in a new method called `countdown_with_sleep` that also takes one integer argument for the countdown.
